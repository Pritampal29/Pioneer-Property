...
jQuery(document).ready(function ($) {
  ...
  /**
   * #####################################################
   *         AJAX Requests for Category Page
   * #####################################################
   */

  $("#advSearchBtnCat").on("click", function () {
    var locationCat = $("#mainLocationCat").val();
    // var roomCat = $("#roomCat").val();
    var statusCat = $("#propStatusCat").val();
    var buyrentCat = $("#propBuyRentCat").val();
    var max_budgetCat = $("#loan-amountCat").val();
    var min_budgetCat = $("#min-budgetCat").val();
    var categoryName = $("#catNameh").text();
    // console.log(buyrentCat);
    // console.log(roomCat);
    var ajaxurl = ajax_object.ajax_url;
    ...
    $.ajax({
      ...
      dataType: "html",
      data: {
        action: "fetch_category_properties",
        locationCat: locationCat,
        // roomCat: roomCat,
        statusCat: statusCat,
        buyrentCat: buyrentCat,
        min_budgetCat: min_budgetCat,
        max_budgetCat: max_budgetCat,
      ...
      success: function (response) {
        $("#mainPropDivCat").html(response);
        $("#mainPropDivCat").fadeIn(500);
        $(".property_loader").removeClass("l_show");
      },
    });
  });
